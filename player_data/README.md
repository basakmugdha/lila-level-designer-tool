...
## Telemetry Viewer (this repo)

A **web-based visualization tool** for Level Design is included in this repo:

- **Backend** (`backend/`): FastAPI app that loads parquet data, converts world coords to minimap pixels, and exposes APIs for maps, matches, match data, and heatmaps.
- **Frontend** (`frontend/`): React app with map canvas, filters (map / date / match), timeline playback, and heatmap overlays (traffic, kills, deaths).
...
export PLAYER_DATA_ROOT=..   # path to folder containing February_10, minimaps, etc.
...
**Deploy on GitHub Pages (live site, no backend):**

1. **Export static data** (once, from your machine where the parquet data lives):
   ```bash
   ...
   ```
   This writes `frontend/public/data/` (index, match JSONs, heatmaps, minimaps). Use `--limit 50` to keep repo size small, or omit for full export.

2. **Commit and push** the repo (including `frontend/public/data/`) to GitHub.

3. **Turn on GitHub Pages:** Repo → **Settings** → **Pages** → **Build and deployment** → Source: **GitHub Actions**.

4. **Trigger deploy:** Push to `main` (or run the “Deploy to GitHub Pages” workflow from the **Actions** tab). After it runs, the site is live at:
   **`https://<your-username>.github.io/<repo-name>/`**

For more detail (troubleshooting, repo name), see [GITHUB_PAGES.md](GITHUB_PAGES.md).

For **deployment** and a **shareable link**, see [DEPLOY.md](DEPLOY.md).  
For **data nuances** (coordinate mapping, encoding, bot detection, timestamps) and **edge cases**, see [DATA_HANDLING.md](DATA_HANDLING.md).  
For **what Level Design can do** in the tool (filters, heatmaps, event toggles) and **gaps**, see [LEVEL_DESIGNER_UX.md](LEVEL_DESIGNER_UX.md).
